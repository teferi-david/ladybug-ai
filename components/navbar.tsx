...
function NavLink({
  ...
  return (
    <Link
      href={href}
      className={cn(
        'text-sm font-medium text-gray-700 transition-colors hover:text-gray-900',
        active && 'text-primary',
        className
      )}
    >
...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 px-3 pt-3 md:px-4 md:pt-4">
      <div className="liquid-glass-nav mx-auto flex max-w-6xl items-center justify-between gap-3 rounded-full border border-white/50 bg-white/55 px-3 py-2.5 shadow-[0_8px_40px_rgba(0,0,0,0.08),inset_0_1px_0_rgba(255,255,255,0.85)] backdrop-blur-2xl md:px-5">
        ...
        <nav className="hidden items-center gap-1 md:flex md:gap-2 lg:gap-6" aria-label="Main">
          <NavLink href="/about">About</NavLink>

          <DropdownMenu>
            <DropdownMenuTrigger
              className={cn(
                'inline-flex items-center gap-1 rounded-full px-2 py-1.5 text-sm font-medium text-gray-700 outline-none transition-colors hover:bg-black/[0.04] hover:text-gray-900 focus-visible:ring-2 focus-visible:ring-primary/30 data-[state=open]:bg-black/[0.05]'
              )}
            >
              Tools
          ...
          {user ? (
            <div className="ml-2 flex items-center gap-2 border-l border-gray-200/80 pl-4 lg:ml-4">
              {showPricingCta && (
                <ProUpgradeButton asChild size="sm" className="min-h-9 rounded-full px-4 text-xs shadow-sm">
                  <Link href="/pricing">Start for free</Link>
                </ProUpgradeButton>
              )}
              <Link href="/settings">
                <Button variant="ghost" size="sm" className="rounded-full text-gray-700">
                  Settings
                </Button>
              </Link>
              <Button variant="outline" size="sm" className="rounded-full border-gray-200/80" onClick={handleSignOut}>
                Sign out
              </Button>
            </div>
          ) : (
            <div className="ml-2 flex items-center gap-2 border-l border-gray-200/80 pl-4 lg:ml-4">
              ...
              <Link
                href="/register"
                className="inline-flex min-h-9 items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground shadow-md shadow-primary/25 transition hover:bg-primary/90"
              >
                Get started
      ...
      {mobileOpen && (
        <div className="liquid-glass-nav mx-auto mt-2 max-w-6xl rounded-2xl border border-white/50 bg-white/70 p-4 shadow-xl backdrop-blur-2xl md:hidden">
