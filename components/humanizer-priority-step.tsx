  ...
  return (
    <div className="min-h-[min(70vh,600px)] px-4 py-10 md:py-16">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <p className="mt-3 text-sm text-gray-600 md:text-base dark:text-zinc-400">
      ...
      <div className="mx-auto mt-10 grid max-w-3xl gap-4 sm:grid-cols-2">
        {HUMANIZE_PRIORITIES.map((p) => (
          <button
            ...
            className={cn(
              'rounded-2xl border-2 bg-white/80 p-5 text-left shadow-sm transition hover:border-primary/40 dark:bg-zinc-950/60',
              selected === p.id ? 'border-primary ring-2 ring-primary/20' : 'border-gray-200/80 dark:border-zinc-700'
            )}
          >
            <p className="font-semibold text-gray-900 dark:text-zinc-100">{p.title}</p>
            <p className="mt-2 text-sm leading-relaxed text-gray-600 dark:text-zinc-400">{p.description}</p>
          </button>
        ))}
      </div>
